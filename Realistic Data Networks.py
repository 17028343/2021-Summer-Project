...
np.random.seed(6)
Gaussian_Noise=np.random.normal(0, 140, size=Training_Set_Size)
...
for i in np.arange(0,3,1):
    for j in np.arange(0,Training_Set_Size,1):
        x_train[j,i]=+np.sqrt(Poisson_Noise[j,i]**2+Gaussian_Noise[j]**2)
...
np.random.seed(13)
Gaussian_Noise_Test=np.random.normal(0, 140, size=Test_Set_Size)
...
for i in np.arange(0,3,1):
    for j in np.arange(0,Test_Set_Size,1):
        x_test[j,i]=+np.sqrt(Poisson_Noise_Test[j,i]**2+Gaussian_Noise_Test[j]**2)
...
Liner=np.linspace(1074.4,1074.8,1000)
GaussianNumber= 88 # within [0,10000]
plt.plot(Liner, 
